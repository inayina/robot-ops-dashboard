...
选项：
  ...
  --no-bridge            只启动 micro-ROS agent，不启动 ROS 2 -> MQTT bridge
...
环境变量：
  ...
  MICRO_ROS_TOPIC_WAIT_SECONDS、MICRO_ROS_MESSAGE_TYPE、MQTT_BROKER_URL、
  MQTT_IMU_TOPIC、ROBOT_ID、DASHBOARD_PORT、FRONTEND_PORT、LOG_DIR
EOF
}
...
ensure_start_paths() {
  ...
  if [[ "$START_DASHBOARD" == true || "$START_BRIDGE" == true ]]; then
...
run_check() {
  ...
  print_mqtt_check
  log "IMU bridge: OK，脚本可编译，默认仅订阅 ROS 2 IMU topic 并镜像到 MQTT $MQTT_IMU_TOPIC。"
  log "自检完成：未启动任何进程。"
}
